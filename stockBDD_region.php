	...
	//Insérer les région : table region
		...
		foreach ($parsed_json_region as $j_region){			
			//Conversion des caractères spéciaux avant insertion dans la BDD
				$r_ID = $parsed_json_region[$cpt]->{'region_id'};
				$r_nom = mysqli_real_escape_string($bdd, $parsed_json_region[$cpt]->{'region_name'});
				$r_pays = mysqli_real_escape_string($bdd, $parsed_json_region[$cpt]->{'region_country_code'});
				$r_lat = $parsed_json_region[$cpt]->{'region_gps_lat'};
				$r_lon = $parsed_json_region[$cpt]->{'region_gps_lon'};
			...
			//Récupérer la liste des salles de cette région	
				...
				if (($r_ID != -1)&&($r_ID != 60)&&($r_ID != 53)){
					foreach ($r_liste_salle as $r_salle){
						echo $r_salle;
						echo '<br/>';
						$sql_region = "INSERT INTO region_salle VALUES('', ".$r_salle.", ".$r_ID.")";
			...
			//Insertion dans la BDD	des données sur les régions
				$sql = "INSERT INTO region VALUES('',".$r_ID." ,'".$r_nom."','".$r_pays."', ".$r_lat.", ".$r_lon.", '".$r_serialize."')";
